...
class DBP15K(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.root.split('/')[-1] == 'DBP15K':
            emb_path = os.path.join(self.root, self.pair, self.pair[:2]+'_vectorList.json')
        if self.root.split('/')[-1] == 'SRPRS':
            emb_path = os.path.join(self.root, self.pair, self.pair[3:] + '_vectorList.json')
        ...
        if self.root == 'data/SRPRS':
            test_path = os.path.join(self.root, self.pair, 'ref_ent_ids')
            train_path = os.path.join(self.root, self.pair, 'sup_ent_ids')
            e1, e2 = read_txt_array(test_path, sep='\t', dtype=torch.long).t()
            test_set = torch.stack([assoc1[e1], assoc2[e2]], dim=0)
            e1, e2 = read_txt_array(train_path, sep='\t', dtype=torch.long).t()
            train_set = torch.stack([assoc1[e1], assoc2[e2]], dim=0)
